...
public class RoomManager : IDisposable
{
    ...
    /// Create a new room
    /// </summary>
    public Room CreateRoom(string roomId, string roomName, IEnumerable<RoomCompatibility> supportedModes, bool isPersistent = false)
    {
    ...
    public bool JoinRoom(uint clientId, string clientName, string roomId)
    {
        ...
        if (room_client != null)
        {
            ...
            if (room.SupportsMode(RoomCompatibility.Spatial))
            {
                room_client.CurrentMode = RoomCompatibility.Spatial;
            }
            // For chat-only rooms, default to TextChat
            else if (room.SupportsMode(RoomCompatibility.TextChat))
            {
                room_client.CurrentMode = RoomCompatibility.TextChat;
            }
            // For voice rooms, default to VoiceChat
            else if (room.SupportsMode(RoomCompatibility.VoiceChat))
            {
                room_client.CurrentMode = RoomCompatibility.VoiceChat;
            }
        }
    ...
    public bool ShouldReceiveSpatialUpdates(uint clientId)
    {
        ...
        return client.CurrentMode == RoomCompatibility.Spatial;
    }
