...
{
    public static readonly Matrix DefaultTransform2DMatrix = new Matrix(3, 3)
    {
        [0,0] = 1,
        [1,1] = 1,
        [2,2] = 1
    ...
    public Matrix(int rows, int columns, float[] values)
    {
        ...
        for (var j = 0; j < m_columns; j++)
            m_matrix[i, j] = values[si++];
    }

    public float[] ToArray()
    {
        var values = new float[m_rows * m_columns];
        var si = 0;
        for (var i = 0; i < m_rows; i++)
        for (var j = 0; j < m_columns; j++)
            values[si++] = m_matrix[i, j];

        return values;
    }

    public Matrix(Matrix matrix):this(matrix.m_rows, matrix.m_columns)
    {
        for(var i = 0; i < matrix.m_rows;i++)
        for (var j = 0; j < matrix.m_columns; j++)
    ...
    public static void Multiply(ref Matrix result, Matrix lmtx, Matrix rmtx)
    {
        if (lmtx.Columns != rmtx.Rows)
            throw new MatrixException("Attempt to multiply matrices with unmatching row and column indexes");
        //return null;
    ...
    public float Determinant()
    {
        ...
        for (int j = 0; j < this.Columns; j++)
        {
            ...
            determinent += (int)System.Math.Pow(-1, j) * this[0, j] * tempMtx.Determinant();
    ...
    public Matrix Adjoint()
    {
        ...
        for (int i = 0; i < this.Rows; i++)
        {
            for (int j = 0; j < this.Columns; j++)
            {
                ...
                adjMtx[j, i] = (int)System.Math.Pow(-1, i + j) * tempMtx.Determinant();
            }
        }
    ...
    public void Round(int precision)
    {
        for (int i = 0; i < this.Rows; i++)
        {
            for (int j = 0; j < this.Columns; j++)
            {
                this[i, j] = (float)System.Math.Round(this[i, j], precision);
            }
        }
    }
    #endregion
