...
export class GameTickUpdateEvent {
  ...
  static serializeCore(writer: MemoryPackWriter, value: GameTickUpdateEvent | null): void {
    ...
    writer.writeArray(value.movementStates, (writer, x) =>
      MovementStateData.serializeCore(writer, x)
    );
    ...
    writer.writeArray(value.hitPlayersState, (writer, x) =>
      HitPlayerStateData.serializeCore(writer, x)
    );
  }
  ...
  static serializeArrayCore(
    ...
    value: (GameTickUpdateEvent | null)[] | null
  ): void {
  ...
  static deserializeCore(reader: MemoryPackReader): GameTickUpdateEvent | null {
    ...
    if (count == 4) {
      ...
      value.hitPlayersState = reader.readArray(reader =>
        HitPlayerStateData.deserializeCore(reader)
      );
    } else if (count > 4) {
      throw new Error(
