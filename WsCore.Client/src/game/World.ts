...
export default class WorldMap {
  ...
  static preload(loader: any) {
    loader.load.image('ground', '/map/ground/Ground.png');
  }

  create(p: Phaser.Scene) {
    this.currentScene = p;

    this.map = p.make.tilemap({
      width: 200,
      height: 200,
      tileWidth: WorldMap.cellSize,
      tileHeight: WorldMap.cellSize,
    });
    var tiles = this.map.addTilesetImage('ground', 'ground', WorldMap.cellSize, WorldMap.cellSize);
